...
class Task(db.Model):
     ...
     def tasks_by_user_today(user=None):
         ...
         tz = UserSettings.get_or_create(user).get_timezone()

         d = datetime.now().replace(tzinfo=pytz.utc).astimezone(tz)
         d1 = datetime(d.year, d.month, d.day, 00, 00, 01)
         d2 = datetime(d.year, d.month, d.day, 23, 59, 59)

         return Task.gql('where user = :1 and due_on >= :2 and due_on <= :3 and finished = False', user, d1, d2)
